...
exports.CustomPatternDecorator = void 0;
const vscode = require("vscode");
const CustomPattern_1 = require("./CustomPattern");
class CustomPatternDecorator {
    ...
    updateConfiguration() {
        ...
        for (const item of configPatterns) {
            ...
            if ((item.foreground !== undefined
                ...
                var pattern = new CustomPattern_1.CustomPattern(item.pattern, (_a = item.patternFlags) !== null && _a !== void 0 ? _a : '', item.highlightEntireLine, item.foreground, item.background, item.fontWeight, item.fontStyle, item.border, item.borderRadius, item.borderSpacing, item.letterSpacing, item.overviewColor, vscode.OverviewRulerLane[item.overviewRulerLane], item.textDecoration);
    ...
    getMatchPositionsInDocument(document, matches) {
    ...
    getMatchPositionsInContentString(content, matches) {
        const matchString = matches[0];
    ...
    getEndPosition(start, newlineCount, matchString) {
        let end;
        if (newlineCount === 0) {
...
exports.CustomPatternDecorator = CustomPatternDecorator;
