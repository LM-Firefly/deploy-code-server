...
export class GitService {
	...
	async getCommitInfo(repoPath: string, commitHash: string) {
		const { stdout: commitInfo } = await execPromise(`git show -s --format=%H%n%an%n%ae%n%ad%n%B ${commitHash}`, { cwd: repoPath });
		const [hash, authorName, authorEmail, date, ...messageParts] = commitInfo.trim().split('\n');
	    const message = messageParts.join('\n');
		
		const { stdout: branches } = await execPromise(`git branch --contains ${commitHash}`, { cwd: repoPath });
		const branchList = branches.trim().split('\n').map(branch => branch.trim().replace(/^\* /, ''));
	
		return { hash, authorName, authorEmail, date, message, branches: branchList };
	}
