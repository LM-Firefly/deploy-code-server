...
(function (FilePickerType) {
    ...
    FilePickerType["FOLDER"] = "FOLDER";
    FilePickerType["BOTH"] = "BOTH";
...
class ServerExplorer {
    ...
    createDeploymentOpenDialogOptions() {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    createOpenDialogOptions(type, label) {
    ...
    addLocationWizardImplementation(serverBean, rspId, client, telemetryProps, startTime) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const defaultName = this.getDefaultServerName(rspId, serverType);
            const fields = [];
            const nameField = {
                id: 'id',
                type: 'textbox',
            ...
            for (const key in req.attributes) {
                ...
                const f1 = this.attrAsFieldDefinition(key, oneAttr, true);
            ...
            for (const key in opt.attributes) {
                ...
                const f1 = this.attrAsFieldDefinition(key, oneAttr, false);
            ...
            const def = {
                ...
                pages: [
                    {
                        ...
                        validator: (parameters) => {
                            ...
                            for (const key in req.attributes) {
                                if (!parameters[key] || parameters[key] === '') {
                                    errors.push({
                                        ...
                                        severity: vscode_wizard_1.SEVERITY.ERROR
                                    });
                                }
                            }
                            // Type validation
                            for (const key in req.attributes) {
                                if (parameters[key] && parameters[key] !== '') {
                                    const err = this.validateWizardDataType(req.attributes[key].type, parameters[key]);
                                    if (err !== null) {
                                        errors.push({
                                            template: { id: `${key}Validation`,
                                                content: (`${key} ${err}`) },
                            ...
                            for (const key in opt.attributes) {
                                if (parameters[key] && parameters[key] !== '') {
                                    const err = this.validateWizardDataType(opt.attributes[key].type, parameters[key]);
                                    if (err !== null) {
                                        errors.push({
                                            template: { id: `${key}Validation`,
                                                content: (`${key} ${err}`) },
                ...
                workflowManager: {
                    ...
                    performFinish(wizard, data) {
                        return __awaiter(this, void 0, void 0, function* () {
                            try {
                                const resp = yield explorer.createServerFullResponse(serverBean, data.id, data, client);
    ...
    quickPickDeploymentType(showQuickPick) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (showQuickPick) {
                const ret = yield vscode_1.window.showQuickPick(['File', 'Exploded'], { placeHolder: 'What type of deployment do you want to add?' });
                if (!ret)
                    return undefined;
    ...
    getChildren(element) {
        if (element === undefined) {
            ...
            const qqq = abc + 3;
            console.log(qqq);
            const tmp = element.serverStates;
            const cloned = [...tmp];
    ...
    getServerStatesByRSP(rspId) {
        const props = this.RSPServersStatus.get(rspId);
